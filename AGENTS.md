...
- Target管理・方法探索では、ユーザーが範囲を限定していなければ全ACTIVE Targetの達成根拠を確認し、未解決の対象は追加改善を探索する。達成根拠が揃ったTargetはisucon-targetがRESOLVEDへ移す。[Work selection](tools/backlog/backlog-workflow.md#work-selection)の優先度は探索順に使い、未探索の対象を除外する理由にしない。実装着手は探索と分け、得点への寄与仮説・状態・Ownerから判断する
