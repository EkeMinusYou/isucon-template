...
For performance regression, distinguish the result of the evaluated implementation from rejection of its improvement hypothesis. Before concluding that added costs cannot be corrected, record the concrete corrections considered and why they do not hold; account for observed measurement effects without inferring unmeasured performance. If the choice of correction or rejection remains unresolved, return the card to INVESTIGATE with its deployed state, evidence, and open questions, and clear its Owner. Temporary restoration or removal is a recovery action, not itself evidence for rejection. See [worker regression handling](../../.agents/skills/isucon-worker/references/post-benchmark.md#悪化時の判断と復旧).
...
For Target management and method exploration, the default scope is every ACTIVE Target. Only an explicit user scope restriction narrows that set; a coordinator's focus or a previous report's priority recommendation determines order, not exclusion. `isucon-objective` likewise reassesses every ACTIVE Objective unless the user limits its scope. Preserve each skill's authority: `isucon-target` assesses attainment and manages Target state, while `isucon-analyze` checks attainment evidence and explores unresolved improvement questions.

For every Target in scope, inspect the current Goal and available Evidence. When the Goal is demonstrated, `isucon-target` transitions it to RESOLVED, subject to relation consistency and inspection-only requests. If relation or ownership coordination prevents the transition, record the attainment evidence, exact adjustment and responsible party; do not confuse that administrative wait with an unmet Goal. Analyze hands attainment evidence to Target management without changing state or automatically invoking another skill. Missing evidence, adoption alone, or lack of candidates is not attainment.

Use Priority, expected score contribution, prior results, and the value of resolving unknowns to order exploration and assign work. Low priority, no new Evidence, or another Target's promising result does not excuse leaving a Target unexplored. An overview or a repeated deferral note is not method exploration. Reuse completed investigations after checking their applicability and remaining independent questions; do not repeat unchanged analysis merely to claim activity. If available resources cannot answer a question, record what was actually inspected, the specific unavailable input or implementation needed, and the condition for resuming it. Continue other independent work. Completion requires an outcome for every Target in scope, not a fixed number of candidates; zero new candidates is acceptable after the questions have been evaluated.

Exploration coverage does not authorize implementation of every candidate. Select implementation work using contribution hypotheses, lifecycle gates, ownership, dependencies and snapshot coherence. ACTIVE or READY status alone is not an implementation reason. When a request combines exploration and implementation, the coordinating parent checks the requested exploration's completion conditions and explains selection across its candidate set before worker starts a new implementation. This includes requested rethink exploration beyond existing Targets. Candidate filing and investigate verification may proceed as findings arrive; implementing a small improvement must not precede this check or substitute for completing exploration. A pending benchmark only pauses decisions that require its result.

If only candidates with small expected contribution to the current score remain, review why promising alternatives were deferred and which questions remain unresolved. Resolve questions that can change selection using available Evidence within the authorized exploration before implementation. When those questions have been evaluated, small improvements may be selected with their expected limits and the reasons no stronger candidate remains. Uncertain effect size is not evidence of small contribution. This check requires neither a large proposal nor an additional configuration-comparison benchmark, and does not extend a standalone worker request into exploration.

Before ending a request spanning multiple skills, the coordinating parent checks coverage of the full user scope, including Targets not handled by the first cluster. Continue actionable questions within the authorized skills. Investigate and worker retain their existing gates, batching rules and implementation scope; this default exploration coverage does not require a standalone worker or rethink to explore every Target. Do not add per-card benchmarks, fixed small batches, or measurement gates. State transitions and additional skill invocations remain governed by their existing authority rules.
