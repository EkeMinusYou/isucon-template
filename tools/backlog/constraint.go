...
func (s *Store) setConstraintLink(constraintID, cardID string, link bool, role, assessmentJSON string, expected int, options mutation, reason string) error {
	...
	if link {
		...
		if strings.TrimSpace(assessmentJSON) != "" {
			...
			parsed = &contract
			if _, contractErr = tx.Exec(`INSERT INTO constraint_intervention_assessments(card_id, constraint_id, assessment_json, constraint_definition_hash, card_definition_hash, updated, updated_by)
				VALUES (?, ?, ?, ?, ?, ?, ?)
				...
				cardID, constraintID, canonical, constraintDefinitionHash(constraint), cardDefinitionHash(card), now(), options.Actor); contractErr != nil {
				tx.Rollback()
				return contractErr
			}
		}
