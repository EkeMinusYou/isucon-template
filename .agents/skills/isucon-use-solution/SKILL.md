...
description: 指定された一つのdocs/solutions文書を現行と照合し、適用候補をINVESTIGATEへ起票する。実装は行わない。
---

# ISUCON solution の適用候補を評価する

担当範囲は指定文書の適用調査・INVESTIGATE起票と報告。READY判定は`isucon-investigate`へ渡し、実装・deploy・benchは行わない。
