...
- Objective/Targetの過去ID・移行判断を辿る必要がある場合は[Objective organization](../../../tools/backlog/objective-organization.md)。移行記録のIDや当時の状態を現行台帳の代用にしない。
...
Objectiveの作成・更新・退役と、その判断根拠の記録を担当する。Targetの内容・状態・Objectiveリンクは[isucon-target](../isucon-target/SKILL.md)の担当とし、Interventionの起票・状態変更は行わない。保存済みEvidence、現行コード・設定、公式資料と必要最小限の読み取り専用SSHを使う。実装、設定変更、deploy、service操作、bench、保存済み成果物の再生成・上書き、当該競技やベンチマーカーのインターネット調査は行わない。

## 手順

1. `task backlog -- objective list`でACTIVE Objectiveと現在のPriorityを確認し、Priorityを調査順の入力として使う。必要なIDを`objective show`で読む。`task backlog -- target list`と関連するopen Intervention、Historyから影響範囲を把握する。退役Objective・終端Targetを管理上確認する場合は、それら自身のshow/Historyだけを読む。Interventionの重複確認・過去調査はopen Interventionだけを対象とし、一覧が必要なら`task backlog -- list --target A-ID`で絞る（`--all`は使わない）。ユーザー指定のObjectiveがあればそれを中心に調べる。
...
3. 既存Objectiveは[寄与仮説の再評価](../_shared/evidence.md#寄与仮説の再評価)に従い、従来仮説と改善後の観測を照合する。寄与経路の比較からPriority（P0/P1/P2）も再評価し、変更する場合は理由をHistoryへ残す。継続・重点変更・先に解く問いの判断と参照するopenカードID・RUNを、対象ObjectiveのHistoryへ残す（検査のみなら更新案に含める）。維持する場合も観測を踏まえた理由を示し、状態変更自体は必須にしない。方針の存続と今回の重点選択を分け、[Work selection](../../../tools/backlog/backlog-workflow.md#work-selection)に沿って次に重視する寄与経路、今回は選ばない経路、その理由をTarget担当へ渡す。観測が不足する関係は、何を確認すれば次の選択が変わるかまで具体化する。
4. 寄与経路の整理からObjectiveの維持・追加・修正・統合・分割・退役を判断する。新規Objectiveを作成する場合、または既存Objectiveを再評価する場合は、Priorityを必ず明示する。独立してスコアへ寄与する仮説は別Objectiveとし、同じ寄与経路内のAPIや改善手段を機械的に分割しない。既存の広いObjectiveに異なる寄与経路をまとめて本文へ追記するだけで終えず、タイトル・定義・評価条件を経路に合わせて再構成する。既存Objectiveを狭める場合は、外れる経路を別Objectiveへ移すか、根拠不足・見送りとして残すかを判断し、黙って落とさない。暗黙の最上位目的、共通の有効性条件、明示要求された別賞の目的はworkflowに従って区別する。
5. 各方針に根拠RUN・観測事実・推定した寄与経路・成立条件・不確実性・評価可能な指標や述語・支持と反証の観測・見直し条件を記録する。Priorityの値と、他Objectiveとの相対的な順序を選んだ理由も簡潔に記録する。得られる利益と追加費用から純寄与を判断し、局所指標の増減だけで得点効果を認定しない。寄与額の確定や個別変更の因果分離をObjective作成の前提にせず、観測に基づく推定を候補選択に使い、後続RUNで更新する。現時点の律速や即時の得点増加も必須条件にしない。
...
ObjectiveのPriorityは、その寄与経路へ次に投資する価値をP0（高）、P1（通常）、P2（低）で表す。得点機会の期待量や損失回避、Evidenceの支持、未確定性、追加される実行時コスト、前提が崩れる条件を相対比較して決める。現在の律速であること、APIが遅いこと、TargetやInterventionが多いことだけではP0にしない。判断に迷う場合はP1とする。

PriorityはObjectiveの状態や正当性要件の代わりにならず、Priorityの高いObjectiveに紐付くTarget・Interventionを自動的に同じ値へしない。ACTIVE Objective全件の詳細確認を省略する理由にも使わず、調査・再評価の順序と次に渡す重点を決めるために使う。明示されたユーザー優先度は保持し、Evidenceで順位が変わった場合だけ変更理由を明記する。

新規作成・再評価後の書込みでは、`objective add` または `objective update` に`--priority P0|P1|P2`を渡す。既存ObjectiveにPriorityが空欄なら、他の定義を黙って変更せず、現在の寄与仮説に対する判断をReasonとHistoryへ残してPriorityだけを更新する。書込み前にObjectiveのversionを再読し、通常どおり`--expect-objective-version`、`--actor skill:isucon-objective`、`--reason`を指定する。
...
RUNを使った判断は対象ObjectiveのCLI Historyへ記録する。RUNがない初期整備はCLIのHistoryと完了報告に残し、架空のRUNを作らない。
