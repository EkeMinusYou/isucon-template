...
無関係なリファクタを混ぜない。一体でrollbackできない独立変更や、境界・因果の再調査が必要になった場合は実装を止め、Ownerを外してINVESTIGATEへ差し戻す。具体的な外部待ちは[workflowのBLOCKED条件](../../../../tools/backlog/backlog-workflow.md#blocked)に従って記録する。技術的反証や安全に成立しないことが判明した場合は、適用済み変更のrollbackを行ってから根拠を記録しREJECTEDにする。
