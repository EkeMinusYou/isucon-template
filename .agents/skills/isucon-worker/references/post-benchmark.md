...
問題がなく[workflowの採用条件](../../../../tools/backlog/backlog-workflow.md#validated-and-rejected)を満たす場合は、`task pass`で対象APPLIEDをVALIDATEDにする。効果量などの結果が曖昧でも、安全性と改善の方向が維持され、同じ採用条件を満たすなら、判断根拠と不確実性をHistoryへ残してVALIDATEDにする。例外採用も同節のFORCE条件に従う。
...
rollback・REJECTEDの判断はworkflowの採否規則に従い、復旧操作は[workerの障害復旧](../SKILL.md#障害復旧)に従う。
