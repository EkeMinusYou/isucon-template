...
悪化時の修正可能性の確認と復旧後の採否は、[悪化時の判断と復旧](references/post-benchmark.md#悪化時の判断と復旧)に従う。一時的な復帰・撤去を不採用の根拠にせず、採否未確定なら再調査へ引き渡す。
...
- ベンチ後モード: 対象APPLIEDを採用、限定修正、不採用、根拠付きの再調査への差し戻しのいずれかへ整理し、APPLIED件数を再確認して継続・終了を判断した。差し戻しは採否未確定として報告する。対象RUNの判定完了だけでは終了しない。
