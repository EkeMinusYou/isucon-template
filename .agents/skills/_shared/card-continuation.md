...
新しいEvidenceが選択の前提を変えた場合は親が範囲と理由を見直す。別の候補があることやAPPLIED枠の空きだけで範囲を広げない。今回選ばないカードは状態・Ownerを変更せず理由と再検討条件を報告し、その詳細調査を終了条件にしない。

ユーザーによる対象・件数の制限と停止指示を優先する。指定件数の処理完了や停止指示があれば終了する。

## 対象がない場合の終了

一度確認し、今回選んだ範囲に取得可能な対象がなければ終了する。Backlog全体が空である必要はない。
